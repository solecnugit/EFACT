  ...
  if node.kind == CursorKind.FUNCTION_DECL or node.kind == CursorKind.CXX_METHOD:
    ...
    for i in node.get_children():
      if i.kind == CursorKind.PARM_DECL:
        argNum += 1
        if i.type.kind == TypeKind.POINTER:
          if re.search('\(\*\)',i.type.spelling):
            argList.append(argNum)
            flag = 1
            print(i.type.spelling)
            print(node.displayname)
        elif i.type.kind == TypeKind.TYPEDEF:
          if re.search('\(\*\)',i.type.get_declaration().underlying_typedef_type.spelling):
            argList.append(argNum)
            flag = 1
            print(i.type.get_declaration().underlying_typedef_type.spelling)
            print(node.displayname)
      
    if flag == 1:
      key = func_name
      FUNCDECL_LIST[key].append(argList)
...
def write_cc_file_with_params(hfile, outfile):
  ...
  with open(outfile, "w") as s:
    #The first layer of loops traverses FUNCDECL_LIST
    for key in FUNCDECL_LIST.keys():
      argList = FUNCDECL_LIST[key]
      s.write("{0} ".format(key))
      for arg in argList:
        s.write("{0} ".format(arg))
      s.write("\n")
