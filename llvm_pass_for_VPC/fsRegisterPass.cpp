  ...
  typedef struct state
{
  ...
  struct RdtscTransplanter : public ModulePass {
    ...
    bool runOnModule(Module &M) {
                ...
                for (GlobalAlias &GA : M.aliases()) {
                  ...
                  std::regex fsRegisterPattern("FSBASE_[0-9a-z]*_[0-9a-z]*");
                  if (std::regex_match(glaName,fsRegisterPattern)) {
                      //遍历该全局变量的users
                      for (User *U : GLa->users()) {
                          ...
                          if (Instruction *Inst = dyn_cast<Instruction>(U)) 
                          {
                              ...
                              //errs() << *Selection_Inst << "\n";
                              //开始对FSBASE进行修改
                              ChangeFsbaseToGlobalAlias(Inst,1,nullptr);
  ...
  ///接受一个对@FSBASE的操作指令
  ///将对FSBASE的操作修改成一个具体的global alias
  void ChangeFsbaseToGlobalAlias(llvm::Instruction *callInst,int rdtsc_flag,llvm::Function *RdtscFunc){
    LLVMContext &Context = callInst->getContext();
    IRBuilder<> Builder(callInst);
    //初始化一个stack，用来保存处理过的指令，最后再统一删除
    std::stack<llvm::Instruction*> instructionStack;

    std::stack<llvm::Instruction*> instructionChangeStack;
    instructionStack.push(callInst);
    //errs() <<&callInst << "\n";

    //获取fs的offset，并基于该offset初始化一个globalAlias
    llvm::Instruction *offsetInst= callInst->getNextNonDebugInstruction();
    instructionStack.push(offsetInst);
    //errs() << *offsetInst << "\n";
    //errs() << *offsetInst->getOperand(0) << "\n";

    //把偏移量取出来放在offset里
    int offset=llvm::dyn_cast<llvm::ConstantInt>(offsetInst->getOperand(0))->getSExtValue();
    //errs() << offset << "\n";
    std::string GlobalFsRegiserName="FSRegister"+std::to_string(offset);
    llvm::GlobalVariable* global_FsRegister_Var;
    llvm::GlobalAlias* fsBase_i64;
    bool global_FSBASE_Var_flag=false;
    bool global_FS_i64_Alias_flag=false;

    //首先查找一下是否已经初始化过了相应的全局变量
    llvm::Module* module = callInst->getModule();
    for (GlobalVariable &GV : module->globals()) {
      GlobalVariable *GLv = dyn_cast<GlobalVariable>(&GV);
      llvm::StringRef GLvName=GLv->getName();
      //遍历全局global variables，如果已经存在了，就不用再创建了
      if (GLvName.str()==GlobalFsRegiserName) {
        global_FsRegister_Var=GLv;
        global_FSBASE_Var_flag=true;
        break;
      }
    }
    ...
    for (GlobalAlias &GA : module->aliases()) {
      GlobalAlias *GLa = dyn_cast<GlobalAlias>(&GA);
      llvm::StringRef GLaName=GLa->getName();
      //遍历全局global alias，如果已经存在了，就不用再创建了
      if (GLaName.str()=="FS_i64") {
        fsBase_i64=GLa;
        global_FS_i64_Alias_flag=true;
        break;
      }
    }
    ...
    //如果没有初始化过，则初始化
    llvm::Constant* global_FsRegister_Const;
    llvm::Type* i64_type = llvm::Type::getInt64Ty(Context);
    if (global_FSBASE_Var_flag==false){
        global_FsRegister_Const = module->getOrInsertGlobal(GlobalFsRegiserName, i64_type);
        global_FsRegister_Var = llvm::dyn_cast<llvm::GlobalVariable>(global_FsRegister_Const);
        global_FsRegister_Var->setInitializer(llvm::Constant::getNullValue(i64_type)); 
        global_FsRegister_Var->setThreadLocal(true); 
        global_FsRegister_Var->setThreadLocalMode(llvm::GlobalVariable::InitialExecTLSModel);
        global_FsRegister_Var->setLinkage(llvm::GlobalVariable::LinkageTypes::PrivateLinkage);
    }
    errs() << *global_FsRegister_Var << "\n";

    
    llvm::Value* fsRegisterBitcast = Builder.CreateBitCast(
        global_FsRegister_Var, 
        llvm::Type::getInt64Ty(Context)
    );
    
    
    /*if(global_FS_i64_Alias_flag==false)
    {
      llvm::PointerType* i64PtrType = llvm::Type::getInt64PtrTy(Context);
      llvm::Constant* fsRegisterPtr = llvm::ConstantExpr::getBitCast(global_FsRegister_Var, i64PtrType);

      fsBase_i64 = llvm::GlobalAlias::create(
          i64_type,
          0, // AddressSpace
    ...
    errs() << *fsBase_i64 << "\n";*/
    Builder.SetInsertPoint(offsetInst);

    /*// Create the alias @FS_i64 to @FSRegister-4
    llvm::PointerType* i64PtrType = llvm::Type::getInt64PtrTy(Context);
    llvm::Constant* fsRegisterPtr = llvm::ConstantExpr::getBitCast(global_FsRegister_Var, i64PtrType);

    llvm::GlobalAlias* fsBase = llvm::GlobalAlias::create(
        i64_type,
        0, // AddressSpace
        llvm::GlobalValue::PrivateLinkage,
        "FS_i64",
    ...
    for (User *U : offsetInst->users()) {
    //逐个对FSBASE的使用进行修改
      if (Instruction *Inst = dyn_cast<Instruction>(U)) 
      {
          Instruction *Selection_Inst =Inst;
          //errs() << *Selection_Inst << "\n";
          //errs() << *Selection_Inst->getOperand(0) << "\n";
          instructionChangeStack.push(Selection_Inst);
          //Selection_Inst->setOperand(0,global_FsRegister_Var);
          //errs() << *Selection_Inst->getOperand(0) << "\n";
          //errs() << *Selection_Inst->getOperand(1) << "\n";
      }
    }
    
    errs() << "进来了！！" << "\n";
    while (!instructionChangeStack.empty()) {
      errs() << "111" << "\n";
      llvm::Instruction* Inst = instructionChangeStack.top();
      instructionChangeStack.pop();
      errs() << *Inst << "\n";
      Inst->setOperand(0,fsRegisterBitcast);
      errs() << *Inst << "\n";
      errs() << "222" << "\n";
    }
  ...
  ///接受一个对@FSBASE的操作指令
  ///将对FSBASE的操作修改成一个具体的global alias
  void rdtsc(llvm::Instruction *callInst,int rdtsc_flag,llvm::Function *RdtscFunc){
    LLVMContext &Context = callInst->getContext();
    //errs() <<&callInst << "\n";
    //获取fs的offset，并基于该offset初始化一个globalAlias
    llvm::Instruction *offsetInst= callInst->getNextNonDebugInstruction();
...
//这边是对pass的注册
// Register for opt
static RegisterPass<RdtscTransplanter> X("fsRegister", "fs Register Solve Pass");
